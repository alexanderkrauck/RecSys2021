...
class RecSys2021TSVDataLoader():
    ...
    def load_next_batch__(self):
        ...
        #can happen that denominator has 0, then replace inf by high value!
        s =  (df["a_follower_count"] / df["b_follower_count"]).replace(np.inf, 10000000)
        df["a_b_follower_ratio"] = s

        s =  (df["b_follower_count"] / df["a_follower_count"]).replace(np.inf, 10000000)
        df["b_a_follower_ratio"] = s

        s = (df["a_following_count"] / df["b_following_count"]).replace(np.inf, 10000000)
        df["a_b_following_ratio"] = s

        s = (df["b_following_count"] / df["a_following_count"]).replace(np.inf, 10000000)
        df["b_a_following_ratio"] = s

...
def load_all_preprocessed_data(
    ...
    if new_features and old_features:
        ...
        return dd.merge(original_df, extra_features_df, how='inner', left_index=True,
                        right_index=True)  # on index, so fast
    elif new_features:
...
def uncompress_and_parquetize(comp_dir: str = COMP_DIR, config: dict = None):
    ...
    uncompressed_dir = os.path.join(comp_dir, mop_config['uncompressed_dir'])
    prepro_dir = os.path.join(comp_dir, mop_config['preprocessed_dir'])

    if data_source == 'comp':
        # decompress lzo files to uncomp directory
        decompress_lzo_file(compressed_dir, uncompressed_dir, delete_compressed=False, overwrite=False,
                            verbose=verbosity)

    # start with reading the files lazily and locally, assume they are uncompressed
    unpacked_files = [os.path.join(uncompressed_dir, f)
                      for f in os.listdir(uncompressed_dir) if os.path.isfile(os.path.join(uncompressed_dir, f))]
    if verbosity >= 2:

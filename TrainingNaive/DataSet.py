...
class NaiveSet(Dataset):
    ...
    def __init__(self, ddf, stats_file=None):
        self.ddf = ddf
        self.stats = stats_file

        # split df and reencode labels
        self.labels = ['has_reply', 'has_retweet', 'has_retweet_comment', 'has_like']
        self.features = [feat for feat in self.ddf.columns if feat not in self.labels]
        self.encodings = [1, 10, 100, 1000]

        for label, code in zip(self.labels, self.encodings):
            self.ddf[label] = self.ddf[label].replace({1: code, 0: 0})

        #to numpy
        self.ddf['class'] = (self.ddf['has_reply'] +
                       self.ddf['has_retweet'] +
                       self.ddf['has_retweet_comment'] +
                       self.ddf['has_like'])
    ...
    def form_subset(self,tuple,dict=None):
        ...
        labels_batch = self.ddf.loc[lst]['class'].values.compute_chunk_sizes()
